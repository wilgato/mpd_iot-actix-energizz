...
#[put("/{id}")]
async fn update_energizz(state: web::Data<AppState>, id: web::Path<i32>, updated_energizz: Json<crate::repository::energizz::EnergizzRequest>) -> ActixResult<impl Responder, ActixError> {
    let energizz = state.energizz_repository.update_energizz(id.into_inner(), updated_energizz).await;
    Ok(web::Json(energizz))
}

#[delete("/{id}")]
async fn delete_energizz(state: web::Data<AppState>, id: web::Path<i32>) -> ActixResult<impl Responder, ActixError> {
    let energizz: DeleteResult = state.energizz_repository.delete_energizz_by_id(id.into_inner()).await;
    Ok(web::Json(json!({
